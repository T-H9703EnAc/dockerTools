...
# PostgresSQLの作業ディレクトリを作成
mkdir $PROJECTNAME/postgres

# PostgreSQL用のDockerfileを作成
echo \
'FROM postgres:14-alpine

ENV LANG ja_JP.utf8' >> $PROJECTNAME/postgres/Dockerfile

# PostgresSQLの初期データディレクトリ
mkdir $PROJECTNAME/postgres/init
...
-- 初期データを挿入
INSERT INTO users VALUES(1,'Tarou');" >> $PROJECTNAME/postgres/init/init.sql
...
VOLUME /spring_project/

WORKDIR /sprin_project/' >> $PROJECTNAME/back/Dockerfile
...
services:
  ...
  back:
    ...
    tty: true
    volumes:
      - ./back/spring_project:/spring_project:cached
    working_dir: /spring_project
  front:
...
# コンテナに入るシェルの作成
echo \
"echo 'postgres --> 1 /back --> 2 / front --> 3'
...
if [ \$SELECTCONTAINER -eq 1 ]; then
    ...
    echo psql -U user -d $DATABASENAME
